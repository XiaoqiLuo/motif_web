...
// selected or not?
if (isset($_GET['source'])||isset($_GET['maximum'])||isset($_GET['minimum'])||isset($_GET['Mouse'])||isset($_GET['Human'])) { // if submitted the choice
...
if (!empty($all_source)) {
  ...
  $query = "SELECT * FROM project where organism='human' and project_home in ('".$all_source."')".$where_pj_limit." order by {$orderBy} desc " . $p->limit();
  ...
  if( $_GET['minimum']=='' && $_GET['maximum']==''){
    $query = "SELECT * FROM project ".$where_pj_unlimit." where organism = 'human' order by {$orderBy} desc " . $p->limit();
    $count = "SELECT count(*) FROM project ".$where_pj_unlimit." where organism = 'human'";
  }else{
    $query = "SELECT * FROM project ".$where_pj_unlimit." and organism = 'human' order by {$orderBy} desc " . $p->limit();
...
<script>
 $(function () {
   $('#head-table').find('th').click(function() {
     ...
     location.href = '/s2172876/motif/project.php?order=' + type
   })
 })
</script>
