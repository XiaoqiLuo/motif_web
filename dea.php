...
if(isset($_GET['submit']) != ''){
    ...
    if($input_keyword!='' && $min=='' && $max==''){
        if($selected_val=='all'){
            ...
            $query = "select * from DEGSummary where ".$selected_val."  LIKE '%".$input_keyword."%' order by {$orderBy} ASC ". $p->limit();
    ...
    if($input_keyword!='' && $min!='' && $max!=''){
        if($selected_val=='all'){
            ...
            # construct query command
            $query = "select * from DEGSummary where ".$selected_val."  LIKE '%".$input_keyword."%' AND ".$selected_numfield." > ".$min." AND ".$selected_numfield." < ".$max." order by {$orderBy} ASC ". $p->limit();
    ...
    if($input_keyword=='' && $min!=''  && $max!=''){
        $query = "select * from DEGSummary where ".$selected_numfield." > ".$min." AND ".$selected_numfield." < ".$max." order by {$orderBy} ASC ". $p->limit();
...
if($input_keyword=='' && $min=='' && $max==''){
        $query = "SELECT * FROM DEGSummary order by {$orderBy} ASC " . $p->limit();
...
<script>
 $(function () {
   $('#head-table').find('th').click(function() {
     ...
     location.href = '/s2172876/motif/dea.php?order=' + type
   })
 })
</script>
