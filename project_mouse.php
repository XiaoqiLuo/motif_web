...
echo '<form action="project_mouse.php" method="GET" >';
echo '<div>';
...
if (!empty($all_source)) {
  ...
  $query = "SELECT * FROM project where organism='mouse' and project_home in ('".$all_source."')".$where_pj_limit." order by {$orderBy} ASC " . $p->limit();
  $count = "SELECT count(*) FROM project where organism='mouse' and project_home  in ('".$all_source."') ".$where_pj_limit;

} else {
  if( $_GET['minimum']=='' && $_GET['maximum']==''){
    $query = "SELECT * FROM project ".$where_pj_unlimit." where organism = 'mouse' order by {$orderBy} ASC " . $p->limit();
    $count = "SELECT count(*) FROM project ".$where_pj_unlimit." where organism = 'mouse'";
  }else{
    $query = "SELECT * FROM project ".$where_pj_unlimit." and organism = 'mouse' order by {$orderBy} ASC " . $p->limit();
    $count = "SELECT count(*) FROM project ".$where_pj_unlimit." and organism = 'mouse'";
  }
  
}
// echo $query;
echo "You are browsing projects (organism:mouse) from ";
if($all_source){
...
<script>
 $(function () {
   $('#head-table').find('th').click(function() {
     ...
     location.href = '/s2172876/motif/project_mouse.php?order=' + type
   })
 })
</script>
